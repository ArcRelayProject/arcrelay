...
export const automationsBridge = {
  ...
  openAutomationScreenPermission: () => invoke("open_automation_screen_permission"),
  pickAutomationPath: (directory: boolean) => invoke("pick_automation_path", { directory }),
  listAutomations: () => invoke("list_automations"),
  saveAutomation: (definition: AutomationDefinition) => invoke("save_automation", { definition }),
  setAutomationEnabled: (automationId: string, enabled: boolean) =>
    invoke("set_automation_enabled", { automationId, enabled }),
  deleteAutomation: (automationId: string) => invoke("delete_automation", { automationId }),
  runAutomation: (automationId: string) => invoke("run_automation", { automationId }),
  testAutomation: (definition: AutomationDefinition, event: AutomationEvent | null = null) =>
    invoke("test_automation", { definition, event }),
  automationCapabilities: () => invoke("automation_capabilities"),
  checkAutomations: (definitions: AutomationDefinition[], capabilities: Capability[]) =>
    invoke("check_automations", { definitions, capabilities }),
  checkAutomation: (definition: AutomationDefinition) => invoke("check_automation", { definition }),
