...
import type {
  ...
  NearbyInputPeer,
  NotificationView,
  PrinterSharingSnapshot,
  PrintJobActivitySnapshot,
  PrivacySettings,
  PrivacySnapshot,
  QuickAction,
  RemoteFileEntry,
  RemoteFileDirectoryPage,
  RemoteFileDragPreparation,
  RemoteFileOpenResult,
  RemoteFileShare,
  RemoteFileState,
...
const cloneState = () =>
  structuredClone({ ...mockData.state, revision: ++mockData.state.revision });
...
function mockShortcutIdentity(value: string) {
  ...
  const aliases: Record<string, string> = {
    ...
    command: "super",
    option: "alt",
    cmdorctrl: mac ? "super" : "ctrl",
    commandorcontrol: mac ? "super" : "ctrl",
  };
  return value
    .toLowerCase()
    .split("+")
...
function browserInputRuntimeSnapshot(): InputRuntimeSnapshot {
  ...
  if (scenario === "recovery" && !mockRecoveredInputPeers.has("remote-office-mac")) {
    const peer = snapshot.nearbyPeers.find(
      (value) => value.serviceInstanceId === "remote-office-mac",
    );
    if (peer) {
      peer.paired = true;
      peer.connected = false;
    }
    snapshot.connectedPeers = snapshot.connectedPeers.filter(
      (value) => value !== "remote-office-mac",
    );
    ...
    if (snapshot.configuration.layout) {
      snapshot.configuration.layout.portals = snapshot.configuration.layout.portals.map(
        (portal) => ({ ...portal, status: "SuspendedOffline" }),
      );
    }
...
function mockRemoteFileTransfer(
...
export const bridge = {
  ...
  ...mcpMockBridge,
  ...automationMock,
  async onPrintJobActivityError(_listener: (message: string) => void): Promise<UnlistenFn> {
    return () => {};
  },
  ...
  async onRuntimeModules(
    _listener: (modules: import("./ipc/generated").ModuleStatus[]) => void,
  ): Promise<UnlistenFn> {
    return () => {};
  },
  isTauri: inTauri,
  ...
  async setInputSharingEnabled(enabled: boolean): Promise<InputRuntimeSnapshot> {
    ...
    for (const display of Object.values(
      mockData.inputRuntime.configuration.layout?.displays ?? {},
    )) {
      if (display.deviceId === mockData.inputRuntime.serviceInstanceId) {
        mockData.inputRuntime.displayAvailability[display.displayId] = enabled
          ? "Ready"
          : "SharingDisabled";
      }
    }
  ...
  async connectInputPeer(
    peer: Pick<NearbyInputPeer, "serviceInstanceId" | "addresses" | "port">,
  ): Promise<InputRuntimeSnapshot> {
    const existing = mockData.inputRuntime.nearbyPeers.find(
      (value) => value.serviceInstanceId === peer.serviceInstanceId,
    );
    if (existing) {
  ...
  async forgetInputPeer(serviceInstanceId: string): Promise<InputRuntimeSnapshot> {
    mockData.inputRuntime.connectedPeers = mockData.inputRuntime.connectedPeers.filter(
      (peer) => peer !== serviceInstanceId,
    );
    mockRecoveredInputPeers.delete(serviceInstanceId);
    mockData.inputRuntime.nearbyPeers = mockData.inputRuntime.nearbyPeers.map((peer) =>
      peer.serviceInstanceId === serviceInstanceId
        ? { ...peer, paired: false, connected: false }
        : peer,
    );
    ...
    if (mockData.inputRuntime.configuration.layout) {
      ...
      mockData.inputRuntime.configuration.layout.portals =
        mockData.inputRuntime.configuration.layout.portals.filter(
          (portal) =>
            !displayIds.includes(portal.sourceDisplay) &&
            !displayIds.includes(portal.targetDisplay),
        );
    }
    mockData.inputRuntime.revision += 1;
    return structuredClone(mockData.inputRuntime);
  ...
  async testEdge(
    ...
    const portal = mockData.inputRuntime.configuration.layout?.portals.find((candidate) => {
      ...
      const edgeMatches = (edge: "Left" | "Right" | "Top" | "Bottom") =>
        edge === "Left"
          ? request.deltaXUm < 0
          : edge === "Right"
            ? request.deltaXUm > 0
            : edge === "Top"
              ...
              : request.deltaYUm > 0;
      return (
        (candidate.sourceDisplay === request.displayId && edgeMatches(candidate.sourceEdge)) ||
        (candidate.direction === "Bidirectional" &&
          candidate.targetDisplay === request.displayId &&
          edgeMatches(candidate.targetEdge))
      );
    });
    ...
    return {
      displayId: portal
        ? reverse
          ? portal.sourceDisplay
          : portal.targetDisplay
        : request.displayId,
  ...
  async takePendingTrayNavigation(): Promise<string | null> {
    return null;
  },
  ...
  async updateAppSettings(patch: import("./ipc/generated").AppSettingsPatch): Promise<AppSettings> {
    ...
    mockData.appSettings = {
      ...
      notifications: { ...mockData.appSettings.notifications, ...patch.notifications },
      sounds: { ...mockData.appSettings.sounds, ...patch.sounds },
      webFiles: { ...mockData.appSettings.webFiles, ...patch.webFiles },
      revision: mockData.appSettings.revision + 1,
    };
    return structuredClone(mockData.appSettings);
  },
  ...
  async previewSound(_event: import("./ipc/generated").SoundEvent): Promise<void> {
    throw new Error("Sound preview is available in the desktop app.");
  },
  async getSoundMuteUntil(): Promise<number | null> {
    return mockSoundMuteUntil;
  },
  async setSoundTemporaryMute(muted: boolean): Promise<number | null> {
    return (mockSoundMuteUntil = muted ? Date.now() + 3_600_000 : null);
  },
  async resetSoundPreferences(): Promise<AppSettings> {
    mockData.appSettings.sounds = defaultSoundPreferences();
    return structuredClone(mockData.appSettings);
  },
  async onSoundMuteChanged(_listener: (until: number | null) => void): Promise<UnlistenFn> {
    return () => {};
  },
  async onDesktopNotification(
    _listener: (notification: { title: string; body: string; error: boolean }) => void,
  ): Promise<UnlistenFn> {
    return () => {};
  },
  ...
  async onOpenSystemShareRequest(
    _listener: (request: import("./types").SystemShareRequest) => void,
  ): Promise<UnlistenFn> {
    return () => undefined;
  },
  ...
  async submitSystemShareRequest(
    _requestId: string,
    _transferId: string,
    _peerId: string,
  ): Promise<void> {},
  ...
  async getTransferState(): Promise<TransferSnapshot> {
    return {
      ...(await transfersMock.getTransferState()),
      discoverable: mockData.appSettings.nearbyDiscoverable,
    };
  },
  ...
  async onPrintJobActivity(
    listener: (value: PrintJobActivitySnapshot) => void,
  ): Promise<UnlistenFn> {
    return () => {};
  },
  ...
  async installRemotePrinter(
    sourceDeviceId: string,
    shareId: string,
  ): Promise<PrinterSharingSnapshot> {
    return {
      hostingSupported: true,
      localPrinters: [],
      shares: [],
  ...
  async listSystemFolders(): Promise<import("./ipc/generated").SystemFolder[]> {
    return [];
  },
  async addSystemFolder(
    _peerId: string,
    _shareId: string,
  ): Promise<import("./ipc/generated").SystemFolder> {
    throw new Error("System folder integration requires the installed desktop app.");
  },
  ...
  async listRemoteDirectory(
    ...
    search = "",
    sortKey: RemoteFileSortKey = "modified",
    sortDirection: RemoteFileSortDirection = "descending",
  ): Promise<RemoteFileDirectoryPage> {
    const offset = Number.parseInt(cursor ?? "0", 10);
    const query = search.trim().toLocaleLowerCase("zh-CN");
    const all = sortRemoteFileEntries(
      mockDirectory(shareId, relativePath).filter((entry) =>
    ...
    return structuredClone({
      entries,
      nextCursor: offset + entries.length < all.length ? String(offset + entries.length) : null,
    });
  },
  ...
  async renameRemoteEntry(
    ...
    if (!entry) throw new Error("item to rename was not found");
    const next = {
      ...entry,
      name: newName,
    ...
    mockData.remoteDirectories.set(
      ...
      entries.map((item) => (item === entry ? next : item)),
    );
    return structuredClone(next);
  },
  async deleteRemoteEntries(
    peerId: string,
    shareId: string,
    paths: string[],
  ): Promise<
  ...
  async deleteRemoteEntry(peerId: string, shareId: string, relativePath: string): Promise<void> {
    ...
    mockData.remoteDirectories.set(
      mockDirectoryKey(shareId, parentPath),
      mockDirectory(shareId, parentPath).filter((item) => item.relativePath !== relativePath),
    );
    return;
  },
  async pickRemoteUpload(
    peerId: string,
    shareId: string,
    relativePath: string,
    folder: boolean,
  ...
  async startRemoteUpload(
    peerId: string,
    shareId: string,
    relativePath: string,
    folder: boolean,
  ): Promise<RemoteFileTransferSession | null> {
  ...
  async startRemoteUploadPaths(
    peerId: string,
    shareId: string,
    relativePath: string,
    paths: string[],
  ): Promise<RemoteFileTransferSession> {
    return mockRemoteFileTransfer(
      "upload",
      peerId,
      shareId,
      relativePath,
      paths.at(0)?.split("/").at(-1) ?? "上传项目",
    );
  },
  async downloadRemoteEntry(
    peerId: string,
    shareId: string,
    relativePath: string,
  ): Promise<string | null> {
  ...
  async downloadRemoteEntries(
    peerId: string,
    shareId: string,
    relativePaths: string[],
  ): Promise<string[] | null> {
    return relativePaths.map(
      (relativePath) => `/Users/demo/Downloads/${relativePath.split("/").at(-1) ?? "download"}`,
    );
  },
  async startRemoteDownloadEntries(
    peerId: string,
    shareId: string,
    relativePaths: string[],
  ): Promise<RemoteFileTransferSession[] | null> {
  ...
  async cancelRemoteFileTransfer(_id: string): Promise<void> {},
  async cancelRemoteThumbnails(_owner: string, _generation: number): Promise<void> {},
  async getRemoteFileThumbnail(
    peerId: string,
    shareId: string,
    relativePath: string,
    modifiedAtMs: number,
  ...
  async openRemoteEntry(
    peerId: string,
    shareId: string,
    relativePath: string,
  ): Promise<RemoteFileOpenResult> {
    return {
      localPath: `/tmp/ArcRelay/remote-open/${relativePath.split("/").at(-1) ?? "file"}`,
      editable: /\.(?:txt|md|json|ya?ml|toml|csv|log|xml|html?|css|js|ts|rs|py|sh)$/i.test(
        relativePath,
      ),
    };
  },
  async prepareRemoteDrag(
    peerId: string,
    shareId: string,
    relativePath: string,
  ): Promise<RemoteFileDragPreparation> {
    const localPath = `/tmp/${relativePath.split("/").at(-1) ?? "download"}`;
    return { localPath, iconPath: localPath };
  },
  async startRemoteFilePromiseDrag(
    peerId: string,
    shareId: string,
    relativePath: string,
  ...
  async setRemoteFileShareWritable(
    shareId: string,
    writable: boolean,
  ): Promise<LocalSharedDirectory> {
    const share = mockData.localShares.find((item) => item.id === shareId);
    if (!share) throw new Error("shared directory not found");
    share.writable = writable;
    return structuredClone(share);
  },
  async setRemoteFileShareWebPolicy(
    shareId: string,
    policy: {
      mode: WebAccessMode;
      listed: boolean;
    ...
    share.web = {
      ...
      ...policy,
      hasPassword:
        policy.mode === "password" ? Boolean(newPassword || share.web.hasPassword) : false,
  ...
  async getWebGatewayStatus(): Promise<WebGatewayStatus> {
    ...
    return {
      ...
      addresses: enabled
        ? [
            `http://192.168.1.20:${mockData.appSettings.webFiles.port}`,
            `http://arcrelay-demo.local:${mockData.appSettings.webFiles.port}`,
          ]
        : [],
  ...
  async listInstalledApps(refresh = false): Promise<InstalledApp[]> {
    return [
      ...
      },
      {
        ...
        identifier: "com.tencent.meeting",
        version: null,
        iconDataUrl: null,
      },
      {
  ...
  async exportActionsText(): Promise<string> {
    return JSON.stringify(
      { format: "arcrelay-actions", version: 1, actions: mockData.state.actions },
      null,
      2,
    );
  },
  ...
  async observeActionOutput(_actionId: string | null): Promise<void> {},
  ...
  async validateActionShortcut(actionId: string, shortcut: string): Promise<void> {
    ...
    const other = mockData.state.actions.find(
      (action) =>
        action.id !== actionId &&
        action.global_shortcut &&
        mockShortcutIdentity(action.global_shortcut) === key,
    );
    ...
    for (const [enabled, value, name] of reserved) {
      if (enabled && mockShortcutIdentity(value) === key)
        throw new Error(`global shortcut is already used by ${name}`);
    }
  },
  async saveAction(action: QuickAction): Promise<BootstrapState> {
    ...
    if (previous && previous.revision !== action.revision)
      throw new Error("Action changed; reopen it before saving");
    const view = {
      ...action,
    ...
    if (index >= 0) mockData.state.actions[index] = view;
    else mockData.state.actions.push(view);
    emitMockState();
    return cloneState();
  },
  ...
  async executeAction(actionId: string): Promise<string> {
    ...
    outputListeners.forEach((listener) =>
      listener({
        actionId,
        revision: outputs.get(actionId)?.length ?? 0,
        lines: outputs.get(actionId) ?? [],
      }),
    );
    return "执行完成";
  },
  ...
  async listNotifications(includeRead = true, limit = 200): Promise<NotificationView[]> {
    return structuredClone(
      mockData.notifications
        .filter((item) => includeRead || item.readAtMs === null)
        .slice(0, limit),
    );
  },
  ...
  async markNotificationRead(notificationId: string): Promise<NotificationView[]> {
    mockData.notifications = mockData.notifications.map((item) =>
    ...
    mockData.state.unreadNotificationCount = mockData.notifications.filter(
      (item) => item.readAtMs === null,
    ).length;
    emitMockState();
    return structuredClone(mockData.notifications);
  },
  ...
  async connectDesktopDevice(deviceId: string): Promise<string> {
    const device = (await bridge.discoverDesktopDevices()).find(
      (item) => item.deviceId === deviceId,
    );
    if (!device) throw new Error("desktop device not found");
  ...
  async forgetPairedDevice(deviceId: string): Promise<void> {
    mockData.state.pairedDevices = mockData.state.pairedDevices.filter(
      (item) => item.id !== deviceId,
    );
    emitMockState();
  },
  async disconnectDevice(deviceId: string): Promise<void> {
    mockData.state.connectedDevices = mockData.state.connectedDevices.filter(
      (device) => device.id !== deviceId,
    );
    emitMockState();
  },
  async setDeviceAutoConnect(deviceId: string, enabled: boolean): Promise<void> {
    mockData.state.pairedDevices = mockData.state.pairedDevices.map((item) =>
      item.id === deviceId ? { ...item, autoConnect: enabled } : item,
    );
    mockData.state.connectedDevices = mockData.state.connectedDevices.map((item) =>
      item.id === deviceId ? { ...item, autoConnect: enabled } : item,
    );
    emitMockState();
  },
