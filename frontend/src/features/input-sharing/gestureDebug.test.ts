...
function batch(id = 1): GestureDebugBatchReport {
  return {
    batch: {
      ...
      totalSamples: 101,
      markerSamples: 100,
      suppressedMarkers: 50,
      droppedSamples: 0,
      capacityDroppedSamples: 0,
      retainedSamples: 1,
      triggers: [],
    },
    samples: [
...
function report(batches = [batch()]): GestureDebugReport {
  return { osVersion: "26.5.2", batches };
}
...
test("gesture laboratory stays implemented but is hidden from the product UI", () => {
  ...
  assert.match(page, /gestureLabVisible\s*&&\s*new URLSearchParams/);
...
test("TXT export keeps all eight numbered actions and metadata without repeated JSON keys", () => {
  ...
  for (let id = 1; id <= 8; id++) {
    assert.ok(
      text.includes(
        `${id},${captureActions[id - 1]},intercept,10000,1,101,1,100,50,0,0,0,超时停止`,
      ),
    );
...
test("unrecognized native fields retain large integers and tiny floats without axis claims", () => {
  const data = report();
  Object.assign(data.batches[0].samples[0], {
    eventType: 29,
    subtype: 6,
    motion: 3225419776,
    progress: 1.401298464324817e-45,
    swipeMask: 2147483648,
  });
...
test("reports actual injection requests separately and preserves zero-event actions and losses", () => {
  ...
  Object.assign(data.batches[0].batch, {
    retainedSamples: 0,
    totalSamples: 3,
    markerSamples: 0,
    suppressedMarkers: 0,
    droppedSamples: 2,
    capacityDroppedSamples: 1,
  });
  data.batches[0].batch.triggers.push({
    elapsedMs: 1500,
    request: {
      action: "swipeUp",
      amount: 0.5,
