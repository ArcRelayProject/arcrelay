<script lang="ts">
  ...
  const requestedSection = new URLSearchParams(window.location.search).get("inputSection");
  let section: Section =
    gestureLabVisible &&
    new URLSearchParams(window.location.search).get("inputDebug") === "gestures"
      ? "gestureDebug"
      : !bridge.isTauri() &&
          ["overview", "workspace", "keyboard", "gaze", "diagnostics"].includes(
            requestedSection ?? "",
          )
  ...
  $: remoteController =
    snapshot?.controller && snapshot.controller !== snapshot.serviceInstanceId
  ...
  async function refresh(silent = false) {
    try {
      ...
      const previewOnboarding =
        !bridge.isTauri() &&
        new URLSearchParams(window.location.search).get("inputScenario") === "onboarding";
      if (
        snapshot &&
        !snapshot.configuration.layout &&
        (previewOnboarding || window.localStorage.getItem("arc-input-onboarding-complete") !== "1")
      ) {
        setupInitialStep = 1;
        showSetup = true;
      }
  ...
  async function toggleInputSharing() {
    ...
    try {
      ...
      notify(
        snapshot.configuration.inputSharingEnabled
          ? uiTranslate("跨屏输入已启用，任一设备都可自然接管", $uiLanguage)
          : uiTranslate("跨屏输入已关闭，输入已安全释放", $uiLanguage),
      );
    } catch (error) {
  ...
  async function reconnectOfflinePeer() {
    ...
    try {
      ...
      notify(
        t("已重新连接{part0}", $uiLanguage, {
          part0: friendlyDeviceName(offlinePeer.serviceInstanceId, snapshot!, $uiLanguage),
        }),
      );
    } catch (error) {
      notify(
        t("重新连接失败：{part0}", $uiLanguage, { part0: diagnosticMessage(error, $uiLanguage) }),
        true,
      );
    } finally {
      busy = false;
  ...
  async function forgetOfflinePeer() {
    ...
    try {
      ...
      notify(
        t("移除设备失败：{part0}", $uiLanguage, { part0: diagnosticMessage(error, $uiLanguage) }),
        true,
      );
    } finally {
      busy = false;
  ...
  onMount(() => {
    ...
    void observeSnapshot(
      ...
      (accept) =>
        bridge.isTauri()
          ? listen<RuntimeSnapshot>("arc-input-state", (event) => accept(event.payload))
          : Promise.resolve(() => {}),
      ...
      .then(() => {
        ...
        if (
          ...
          !snapshot.configuration.layout &&
          (previewOnboarding ||
            window.localStorage.getItem("arc-input-onboarding-complete") !== "1")
        ) {
          setupInitialStep = 1;
          showSetup = true;
        }
...
<main
  class="input-sharing-host content full-content"
  class:diagnostics-view={section === "diagnostics"}
>
  {#if section === "gestureDebug"}
    <header class="input-sharing-header">
      <div>
        <h1>{uiTranslate("多指手势实验", $uiLanguage)}</h1>
        <p>{uiTranslate("本机采集、短时拦截与原生手势合成 · 临时诊断页", $uiLanguage)}</p>
      </div>
      <button class="button secondary" on:click={() => (section = "overview")}
        >{uiTranslate("返回跨屏输入", $uiLanguage)}</button
      >
    </header>
    <div class="gesture-debug-content"><GestureDebugPage inputSharingEnabled={enabled} /></div>
  {:else}
    {#if gestureLabVisible}
      <div class="gesture-debug-entry">
        <button class="button secondary" on:click={() => (section = "gestureDebug")}
          ><Bug size={16} /> {uiTranslate("手势实验（本机）", $uiLanguage)}</button
        >
      </div>
    {/if}
    {#if loading}
      <div class="input-loading">
        <span></span>
        <p>{uiTranslate("正在建立跨屏输入工作区…", $uiLanguage)}</p>
      </div>
    {:else if snapshot}
      {#if showSetup}
        <InputSetupFlow
      ...
      {:else}
        <header class="input-sharing-header">
          <div>
            <div class="title-line">
              ...
              <span class:controlling class:warning={Boolean(offlinePeer)}
                >{uiTranslate(status, $uiLanguage)}</span
              >
            </div>
            <p>
              {uiTranslate(
                section === "workspace"
                  ? "按桌面上的真实位置排列屏幕，相邻重叠边段会自动连通。"
                  : section === "keyboard"
                    ? "让快捷键在不同系统上保持相同意图。"
                    : section === "gaze"
                      ? "用本机摄像头预选正在注视的屏幕与位置。"
          ...
          <div class="header-control">
            <small
              >{uiTranslate(
                offlinePeer
                  ? t("等待{part0}重新连接", $uiLanguage, {
                      part0: friendlyDeviceName(
                        offlinePeer.serviceInstanceId,
                        snapshot!,
                        $uiLanguage,
                      ),
                    })
                  : enabled
                    ? "任一设备活动即可自动接管 · ⌘⌥⇧ Esc 紧急释放"
                    : "启用一次后持续待命",
                $uiLanguage,
              )}</small
            >
            <button
              ...
              title={uiTranslate(
                !canTakeInputControl(snapshot)
                  ? (snapshot.capabilities.limitation ?? "需要开启辅助功能权限")
                  : "",
                $uiLanguage,
              )}
            ...
            >
              ...
              {uiTranslate(
                busy ? "处理中…" : enabled ? "关闭跨屏输入" : "启用跨屏输入",
                $uiLanguage,
              )}
            </button>
          </div>
          ...
          {#each sections as item}
            <button class:active={section === item.id} on:click={() => (section = item.id)}>
              <svelte:component
                this={item.icon}
                size={18}
                weight={section === item.id ? "fill" : "regular"}
              />
        ...
        {#if snapshot.controller}
          <div class="control-session" role="status" aria-live="polite">
            ...
            <div>
              <strong
                >{t("当前输入来自：{name}", $uiLanguage, {
                  name: controlling
                    ? uiTranslate("这台设备", $uiLanguage)
                    : (remoteController ?? uiTranslate("目标设备", $uiLanguage)),
                })}</strong
              ><small
                >{uiTranslate(
                  "检测到另一台设备的本地输入时会自动切换 · 会话已加密",
                  $uiLanguage,
                )}</small
              >
            </div>
            {#if section !== "overview"}<button
                class="session-secondary"
                on:click={() => (section = "overview")}
                ><ArrowLeft size={15} />{uiTranslate("切回总览", $uiLanguage)}</button
              >{/if}
          </div>
        {/if}
        ...
        <div class="input-sharing-content">
          {#if section === "overview"}
            <InputOverviewPage
              ...
              onNavigate={navigate}
              onSetup={openSetup}
              onToggleInputSharing={toggleInputSharing}
              onRetry={reconnectOfflinePeer}
              onForget={forgetOfflinePeer}
            />
    ...
    {:else}
      <div class="input-loading">
        <p>{uiTranslate("跨屏输入运行时暂不可用。", $uiLanguage)}</p>
        <button class="button" on:click={() => refresh()}>{uiTranslate("重试", $uiLanguage)}</button
        >
      </div>
    {/if}
  {/if}
...
<style>
  ...
  .gesture-debug-entry button {
    display: flex;
    align-items: center;
    gap: 6px;
  }
  .gesture-debug-content {
    min-height: 0;
    flex: 1;
    overflow: auto;
  }
  .input-sharing-header {
    display: flex;
    min-height: 40px;
    align-items: center;
  ...
  .title-line span.controlling {
    color: var(--accent-strong);
    background: var(--accent-soft);
  }
  .title-line span.warning {
    color: var(--warning);
  ...
  .control-session strong {
    flex: none;
    font-size: 12px;
  }
  .control-session small {
    min-width: 0;
    overflow: hidden;
    color: var(--text-secondary);
    font-size: 10px;
  ...
  .input-sharing-content {
    min-height: 0;
    flex: 1;
    overflow: auto;
    padding-top: 12px;
  }
  .diagnostics-view .input-sharing-content {
    padding-right: 4px;
    scrollbar-gutter: stable;
  }
  .diagnostics-view .control-session {
    min-height: 34px;
    margin-top: 8px;
    border-color: var(--border);
    background: var(--surface-soft);
  }
  .diagnostics-view .control-session strong {
    flex-shrink: 1;
    overflow-wrap: anywhere;
    font-size: 11px;
  }
  .diagnostics-view .session-secondary {
  ...
  :global(.input-sharing-host button) {
    font-family: inherit;
    cursor: pointer;
  }
  :global(.input-sharing-host button:disabled) {
    cursor: default;
  }
  :global(.input-sharing-host .card) {
    border-color: var(--border);
  ...
  :global(.input-sharing-host .button:hover) {
    background: var(--accent-strong);
  }
  :global(.input-sharing-host .button.secondary) {
    border-color: var(--border-strong);
    color: var(--text);
    background: var(--surface);
  }
  :global(.input-sharing-host .button.danger) {
    border-color: var(--danger);
    background: var(--danger);
  }
  :global(.input-sharing-host .muted) {
    color: var(--text-secondary);
  }
  :global(.input-sharing-host .mono) {
    font-family: ui-monospace, SFMono-Regular, Menlo, monospace;
  }
  :global(.input-sharing-host .badge) {
    display: inline-flex;
    border-radius: 999px;
    padding: 4px 9px;
    color: var(--accent-strong);
  ...
  :global(.input-sharing-host .field) {
    display: grid;
    gap: 6px;
    color: var(--text-secondary);
    font-size: 12px;
  }
  :global(.input-sharing-host .field input),
  :global(.input-sharing-host .field select) {
    min-height: 38px;
