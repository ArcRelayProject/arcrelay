<script lang="ts">
  ...
  async function refreshCameras() {
    ...
    try {
      ...
      notify(`摄像头枚举失败：${diagnosticMessage(error, $uiLanguage)}`, true);
    } finally {
      busy = false;
    }
  }
  ...
  async function installModels() {
    ...
    try {
      ...
      notify(`安装眼动模型失败：${diagnosticMessage(error, $uiLanguage)}`, true);
    } finally {
      busy = false;
    }
  }
  ...
  async function removeModels() {
    ...
    try {
      ...
      notify(`移除眼动模型失败：${diagnosticMessage(error, $uiLanguage)}`, true);
    } finally {
      busy = false;
    }
  }
  ...
  async function start(): Promise<boolean> {
    ...
    try {
      ...
      notify(`启动眼动追踪失败：${diagnosticMessage(error, $uiLanguage)}`, true);
      return false;
    } finally {
      busy = false;
    }
  ...
  async function launchCalibration() {
    ...
    try {
      ...
      phase = "ready";
      notify(`无法开始标定：${diagnosticMessage(error, $uiLanguage)}`, true);
    } finally {
      launchBusy = false;
    }
  }
  ...
  async function sendFlow(
    ...
    try {
      ...
      notify(
        `无法在 ${active.display.name} 显示标定圆点：${diagnosticMessage(error, $uiLanguage)}`,
        true,
      );
      await cancelCalibration(false);
    } finally {
  ...
  async function captureTarget() {
    ...
    try {
      ...
      stableSince = 0;
      dwellProgress = 0;
      notify(`自动采样失败：${diagnosticMessage(error, $uiLanguage)}`, true);
  ...
  async function beginPresenceEnrollment() {
    ...
    try {
      ...
      notify(`无法开始录入：${diagnosticMessage(error, $uiLanguage)}`, true);
    } finally {
      busy = false;
    }
  }
  ...
  {:else}
        ...
        <div>
          ...
          {:else}
            <button class="start-button" disabled={busy} on:click={installModels}
              >{status?.modelPack.state === "damaged" ? "重新下载" : "下载并启用"}</button
            >
          {/if}
    ...
    <div class="stepper" aria-label="标定步骤">
      <span
        ...
        ><i>{phase === "ready" || phase === "checking" ? "1" : "✓"}</i>准备</span
      ><b></b>
      <span
        class:active={phase === "calibrating" || phase === "transition"}
        class:done={phase === "complete"}><i>{phase === "complete" ? "✓" : "2"}</i>标定</span
      ><b></b>
    ...
    {#if phase === "ready"}
      <article class="hero-card">
        <div class="hero-copy">
          ...
          <p>
            ArcRelay
            会同时学习头部方向和个人眼动偏差；每个点采集多帧，并在不确定时保持当前屏幕，避免误切。
          </p>
          <label class="camera-field"
            ><span>用于标定的摄像头</span><AppSelect
              bind:value={selectedCamera}
              options={cameraOptions}
              disabled={!modelsReady || running || busy}
              placeholder={modelsReady ? "未发现摄像头" : "请先下载眼动模型"}
              aria-label="用于标定的摄像头"
            /></label
          >
          <div class="privacy-line">
            <LockKey size={15} weight="fill" /><span
              ><strong>完全本机处理</strong
              >　只同步圆点位置与进度，摄像头画面和人脸特征不会离开本机。</span
            >
          </div>
          <div class="hero-actions">
            {#if status?.calibrated && !running}
              <button
                ...
                ><Play size={17} weight="fill" />{busy ? "正在启动…" : "启动识别"}</button
              >
              <button class="text-button" disabled={busy} on:click={() => beginCalibration()}
                ><ArrowClockwise size={15} />全部重新标定</button
              >
            {:else if !status?.calibrated}
              <button
                ...
                ><Play size={17} weight="fill" />{busy ? "正在启动…" : "开始标定"}</button
              >
            {:else}
              <button class="text-button" disabled={busy} on:click={() => beginCalibration()}
                ><ArrowClockwise size={15} />全部重新标定</button
            ...
            <button
              ...
              on:click={() => (diagnosticsOpen = true)}><Camera size={15} />视觉诊断</button
            >
            <button class="text-button" disabled={!modelsReady || busy} on:click={refreshCameras}
              ><ArrowClockwise size={15} />重新检测摄像头</button
            >
            {#if modelsReady}<button
                class="text-button"
                disabled={busy || running}
                on:click={removeModels}><Trash size={15} />移除模型</button
              >{/if}
          </div>
        </div>
        <div class="preview-panel" aria-hidden="true">
          ...
          <div class="preview-meta">
            <span><Monitor size={15} />{allDisplays.length} 块屏幕</span><span
              >{deviceCount} 台设备</span
            ><span><Eye size={15} />头部 + 眼动</span>
          </div>
        </div>
      </article>
      <div class="readiness-grid">
        <section>
          ...
          <div>
            <strong>摄像头</strong>
            <p>{cameras.length ? `${cameras.length} 个设备可用` : "等待摄像头权限"}</p>
          </div>
        </section>
        <section>
          ...
          <div>
            <strong>标定范围</strong>
            <p>{allDisplays.length} 块屏幕 · {deviceCount} 台设备</p>
          </div>
        </section>
        <section>
          ...
          <div>
            <strong>眼动服务</strong>
            <p>{running ? `${status?.cameraName ?? "摄像头"} 正在运行` : "开始标定时自动启动"}</p>
          </div>
        </section>
      </div>
      {#if status?.calibrated}
        <section class="existing-profile">
          ...
          <div class="profile-copy">
            <strong>标定已保存，重启后可直接恢复</strong>
            <p>某块屏幕识别不准时，只优化这一块即可保留其他屏幕的数据。</p>
          </div>
            ...
            {#each allDisplays as display}
              <span class:missing={!calibratedDisplayIds.has(display.displayId)}>
                ...
                <button
                  ...
                  >{calibratedDisplayIds.has(display.displayId) ? "优化标定" : "补充标定"}</button
                >
              </span>
            {/each}
          </div>
          <button class="delete-profile" on:click={clearCalibration}
            ><Trash size={15} />删除全部</button
          >
        </section>
      {/if}
      <section class:recognized={status?.presenceState === "ownerPresent"} class="presence-card">
        ...
        <div class="profile-copy">
          ...
          <p>
            {status?.presenceProfileEnrolled
              ? `${status.presenceProfileName ?? "本机用户"} · ${presenceLabel}`
        ...
        {#if status?.presenceEnrollmentActive}
          <div class="presence-progress">
            <span
              >{presencePoseLabel} · {status.presenceEnrollmentSamples}/{status.presenceEnrollmentRequiredSamples}</span
            >
            <div class="bar">
              <span
        ...
        {:else if status?.presenceProfileEnrolled}
          <div class="presence-state">
            <b>{presenceLabel}</b><span
              >{status.presenceFaceCount} 张人脸{status.presenceOwnerSimilarity == null
                ? ""
                : ` · 相似度 ${status.presenceOwnerSimilarity.toFixed(2)}`}</span
            >
          </div>
          <button class="delete-profile" on:click={clearPresenceProfile}
            ><Trash size={15} />删除模板</button
          >
        {:else}
          <label class="presence-name"
            ><span>显示名称</span><input bind:value={presenceName} maxlength="80" /></label
          >
          <button
            class="start-button"
            disabled={busy || !selectedCamera}
            on:click={beginPresenceEnrollment}>开始录入</button
          >
        {/if}
      </section>
    {:else if phase === "checking"}
      <article class="state-card">
        ...
        <span class="eyebrow">自动环境检查</span>
        <h2>{faceReady ? "很好，请保持这个姿势" : "请面向摄像头"}</h2>
        <p>
          {faceReady
        ...
        <div class="check-list">
          <span class:ok={Boolean(status?.faceConfidence && status.faceConfidence >= 0.6)}
            ><i></i>面部清晰</span
          ><span class:ok={Boolean(status?.observation)}><i></i>头部方向有效</span><span
            class:ok={faceReady}><i></i>姿态稳定</span
          >
        </div>
    ...
    {:else if phase === "calibrating" || phase === "transition"}
      <article class="state-card">
        ...
        <span class="eyebrow">跨设备全屏标定</span>
        <h2>
          {phase === "transition"
            ? "正在切换到下一块屏幕"
            : `请看向 ${target?.display.name ?? "屏幕"} 上的圆点`}
        </h2>
        <p>
          {phase === "transition"
            ? "下一台设备会自动显示引导，不需要移动或点击窗口。"
            : `${samplingMode} · 自然注视圆点并保持坐姿；面部或眼动不稳定时会自动暂停。`}
        </p>
        ...
        >
        <button class="cancel-button" on:click={() => cancelCalibration()}
          ><Stop size={15} />退出标定</button
        >
      </article>
    {:else}
      <article class="state-card complete-card">
        <div class="complete-icon"><Check size={38} weight="bold" /></div>
        <span class="eyebrow">标定完成</span>
        <h2>看向屏幕即可移动鼠标</h2>
        <p>
          ArcRelay
          会融合头部方向与眼动选择屏幕；只有结果明确且持续稳定时才会切换，不确定时保持当前屏幕。
        </p>
        ...
        <div class="hero-actions">
          <button class="start-button" on:click={() => (phase = "ready")}>返回眼动设置</button
        ...
        <div class="recognition-screens" aria-label="屏幕识别状态">
          {#each allDisplays as display}
            <span class:active={status?.target?.displayId === display.displayId}
              ...
              >{#if status?.target?.displayId === display.displayId}<i>正在看</i>{/if}</span
            >
          {/each}
        </div>
        <dl>
    ...
    <div class="safety-note">
      <strong>自动切屏：</strong>稳定看向另一块屏幕后，ArcRelay 会移动鼠标到屏幕中央；按 ⌘⌥⇧ Esc
      可随时紧急释放，不会自动点击或输入。
    </div>
  {/if}
</section>
