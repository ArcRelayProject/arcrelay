...
<div class="profiles">
  <nav class="profile-list card">
    ...
    {#each profiles as item}
      <button
        class:active={item.kind === profile?.kind}
        on:click={() => (selectedKind = item.kind)}
      >
  ...
  {#if profile}
    <div class="editor card">
      <div class="editor-header">
        <div>
          <span class="section-label">{uiTranslate("规则优先级已锁定", $uiLanguage)}</span>
          <h2>{keyboardProfileName(profile, $uiLanguage)}</h2>
        </div>
        <label class="field"
          >{uiTranslate("文字策略", $uiLanguage)}
          <AppSelect
            bind:value={profile.textStrategy}
            onValueChange={onChange}
            aria-label={uiTranslate("文字策略", $uiLanguage)}
            options={[
      ...
      <p class="muted explanation">
        {uiTranslate(
          "临时 Raw 模式 → 目标应用 Profile → 用户规则 → 设备对规则 → 系统预设 → 物理键直通。映射从首个按键按下锁定到整个组合键释放。",
          $uiLanguage,
        )}
      </p>
      <div class="rules">
        ...
        <MappingRuleEditor
          source="Windows Alt+Tab"
          target="macOS Command+Tab"
          description="切换应用"
        />
      ...
      <div class="custom-heading">
        <h3>{uiTranslate("自定义语义规则", $uiLanguage)}</h3>
        <button class="button secondary" on:click={addRule}
          >{uiTranslate("添加规则", $uiLanguage)}</button
        >
      </div>
      <div class="rules">
        ...
        {/each}
        {#if profile.semanticOverrides.length === 0}<p class="muted">
            {uiTranslate("尚无自定义覆盖，将使用系统组合预设。", $uiLanguage)}
          </p>{/if}
      </div>
      <KeyboardTester />
    </div>
