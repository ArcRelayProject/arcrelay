...
import type { ClipboardDragRequest, ClipboardDragEnded } from "./dragSession";
...
export const clipboardBridge = {
  ...
  cancelDrag: (token: string) => invoke("clipboard_cancel_drag", { token }),
  onDragEnded: async (handler: (event: ClipboardDragEnded) => void): Promise<UnlistenFn> =>
    listen("clipboard-drag-ended", (event) => handler(event.payload)),
  ...
  ): Promise<UnlistenFn> => listen("clipboard-navigation-key", (event) => handler(event.payload)),
  onNavigationPaused: async (handler: () => void): Promise<UnlistenFn> =>
    listen("clipboard-navigation-paused", handler),
  ...
  ) => invoke("clipboard_text_preview", { id: item.id, format }),
  ...
  setFavorite: (id: number, favorite: boolean) =>
    invoke("clipboard_set_favorite", { id, favorite }),
  ...
  updateLabel: (labelId: string, name: string, color: string) =>
    invoke("clipboard_update_label", { labelId, name, color }),
  ...
  setLabelMembership: (id: number, labelId: string, attached: boolean) =>
    invoke("clipboard_set_label_membership", { id, labelId, attached }),
  ...
  onChanged: async (handler: () => void): Promise<UnlistenFn> =>
    listen("clipboard-changed", handler),
  onOcrChanged: async (handler: (id: number) => void): Promise<UnlistenFn> =>
    listen("clipboard-ocr-changed", (event) => handler(event.payload)),
  onShown: async (handler: () => void): Promise<UnlistenFn> =>
    listen("clipboard-window-shown", handler),
  onHidden: async (handler: () => void): Promise<UnlistenFn> =>
    listen("clipboard-window-hidden", handler),
  onPinChanged: async (handler: (pinned: boolean) => void): Promise<UnlistenFn> =>
    listen("clipboard-window-pin-changed", (event) => handler(event.payload)),
  onContinuousPasteError: async (handler: (error: string) => void): Promise<UnlistenFn> =>
    listen("clipboard-continuous-paste-error", (event) => handler(event.payload)),
};
