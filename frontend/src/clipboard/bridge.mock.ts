...
import type {
  ...
  NearbyClipboardPeer,
  ClipboardPasteMode,
  ContinuousPasteItemInput,
} from "./types";
import type { ClipboardSortPreference } from "../types";
import type { ClipboardTimeline, ClipboardTimelinePosition } from "./types";
import type {
  ClipboardDragRequest,
  PreparedClipboardDrag,
  ClipboardDragEnded,
} from "./dragSession";
const dragListeners = new Set<(event: ClipboardDragEnded) => void>();
const preparedDrags = new Map<string, ClipboardDragRequest>();
export function emitMockDragEnded(event: ClipboardDragEnded) {
  preparedDrags.delete(event.token);
  dragListeners.forEach((handler) => handler(event));
}
...
const mockItems: ClipboardItem[] = [
  ...
  },
  {
    ...
    safeHtml:
      "<h3>设备列表</h3><ul><li><strong>陈子的 MacBook Pro</strong> · 在线</li><li>iPhone 15 Pro · 在线</li></ul>",
...
function mockHistory(
  ...
  const entries = mockItems.filter(
    (item) =>
      (!kind || item.kind === kind || (kind === "text" && item.kind === "html")) &&
      (!favoriteOnly || item.favorite) &&
      (labelIds.length === 0 ||
        labelIds.every((labelId) => item.labels.some((label) => label.id === labelId))) &&
      (!query ||
        `${item.preview} ${item.sourceApp ?? ""}`.toLocaleLowerCase("zh-CN").includes(query)),
  );
...
export const clipboardBridge = {
  prepareDrag: async (request: ClipboardDragRequest): Promise<PreparedClipboardDrag> => {
    ...
    return {
      token,
      kind:
        request.mode === "text_file" ||
        request.ids.some((id) =>
          ["image", "files"].includes(mockItems.find((item) => item.id === id)?.kind ?? ""),
        )
          ? "files"
          : "text",
  ...
  startDrag: async (token: string): Promise<void> => {
    if (!preparedDrags.has(token)) throw new Error("Drag preparation expired");
    // Browser fixtures cannot hand files to the OS. Tests finish this session explicitly.
    window.dispatchEvent(new CustomEvent("mock-clipboard-drag-started", { detail: { token } }));
  },
  cancelDrag: async (token: string): Promise<void> => {
    emitMockDragEnded({ token, outcome: "cancelled" });
  },
  onDragEnded: async (handler: (event: ClipboardDragEnded) => void): Promise<UnlistenFn> => {
    dragListeners.add(handler);
    return () => {
      dragListeners.delete(handler);
    };
  },
  ...
  onNavigation: async (
    handler: (key: import("../ipc/generated").ClipboardNavigationKey) => void,
  ): Promise<UnlistenFn> => {
  ...
  timeline: async (
    ...
    limit = 30,
  ): Promise<ClipboardTimeline | null> => {
    const timestamp = (item: ClipboardItem) =>
      sortBy === "createdAt" ? item.firstCapturedAtMs : item.updatedAtMs;
    const sorted = [...mockItems].sort(
      (a, b) =>
        timestamp(b) - timestamp(a) || (b.syncId > a.syncId ? 1 : b.syncId < a.syncId ? -1 : 0),
    );
    const cursorFor = (item: ClipboardItem): ClipboardCursor => ({
      sortAtMs: timestamp(item),
      id: item.id,
    });
    const after = (item: ClipboardItem, cursor: ClipboardCursor) =>
      timestamp(item) > cursor.sortAtMs ||
      (timestamp(item) === cursor.sortAtMs &&
        item.syncId > (mockItems.find((row) => row.id === cursor.id)?.syncId ?? ""));
    const before = (item: ClipboardItem, cursor: ClipboardCursor) =>
      timestamp(item) < cursor.sortAtMs ||
      (timestamp(item) === cursor.sortAtMs &&
        item.syncId < (mockItems.find((row) => row.id === cursor.id)?.syncId ?? ""));
    ...
    if (position.type === "around") {
      ...
      entries = sorted.filter((item) =>
        position.type === "newer" ? after(item, position.cursor) : before(item, position.cursor),
      );
    ...
    return {
      ...
      anchor,
      newerCursor:
        first && sorted.some((item) => after(item, cursorFor(first))) ? cursorFor(first) : null,
      olderCursor:
        last && sorted.some((item) => before(item, cursorFor(last))) ? cursorFor(last) : null,
  ...
  textPreview: async (
    item: ClipboardItem,
    format: import("../ipc/generated").ClipboardTextFormat | null = null,
  ): Promise<import("../ipc/generated").ClipboardTextPreview> => ({
  ...
  startContinuousPaste: (items: ContinuousPasteItemInput[]) =>
    Promise.resolve({
      ...
      nextPreview: items[0]?.preview ?? null,
  ...
  onChanged:
    async (handler: () => void): Promise<UnlistenFn> =>
    () =>
      undefined,
  onOcrChanged:
    async (handler: (id: number) => void): Promise<UnlistenFn> =>
    () =>
      undefined,
  onShown:
    async (handler: () => void): Promise<UnlistenFn> =>
  ...
  onHidden:
    ...
    () =>
      undefined,
  onPinChanged:
    async (handler: (pinned: boolean) => void): Promise<UnlistenFn> =>
    () =>
      undefined,
  onContinuousPasteError:
    async (handler: (error: string) => void): Promise<UnlistenFn> =>
    () =>
      undefined,
};
