...
export const filesBridge = {
  ...
  addSystemFolder: (peerId: string, shareId: string) =>
    invoke("add_system_folder", { peerId, shareId }),
  ...
  cancelRemoteFileTransfer: (id: string) => invoke("cancel_remote_file_transfer", { id }),
  cancelRemoteThumbnails: (owner: string, generation: number) =>
    invoke("cancel_remote_thumbnails", { owner, generation }),
  async getRemoteFileThumbnail(
    ...
    return invoke("get_remote_file_thumbnail", {
      peerId,
      shareId,
      relativePath,
      modifiedAtMs,
