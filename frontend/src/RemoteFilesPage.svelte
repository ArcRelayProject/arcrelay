<script lang="ts">
  ...
  import { fileWindow, nextFileGeneration, RequestOwner } from "./remoteFileWindow";
  ...
  let treeDirectoryVersions = new Map<string, number>();
  const treeOwner = new RequestOwner();
  const thumbnailOwner = "remote-files-main";
  let thumbnailGeneration = nextFileGeneration();
  let viewportHeight = 800;
  let viewportWidth = 1000;
  let viewportTop = 0;
  let scrollNode: HTMLDivElement;
  $: if (scrollNode) viewportTop = scrollNode.scrollTop;
  let thumbnailRunning = 0;
  let thumbnailQueue: RemoteFileEntry[] = [];
  $: visibleWindow = fileWindow(
    entries.length,
    viewportTop,
    viewportHeight,
    viewportWidth,
  ...
  $: visibleEntries = entries.slice(visibleWindow.start, visibleWindow.end);
  $: queueVisibleThumbnails(visibleEntries, selectedPeerId, selectedShareId, thumbnailGeneration);

  function invalidateThumbnails() {
    thumbnailGeneration = nextFileGeneration();
    thumbnailQueue = [];
    void bridge.cancelRemoteThumbnails(thumbnailOwner, thumbnailGeneration).catch(console.error);
  }
  ...
  $: allVisibleSelected =
    ...
    sortedEntries.every((entry) => selectedPaths.has(entry.relativePath));
  $: selectionSize = selectedEntries.reduce(
  ...
  onMount(() => {
    ...
    if (bridge.isTauri()) {
      ...
      void webviewWindow
        .scaleFactor()
        .then((factor) => (webviewScaleFactor = factor))
        .catch(() => undefined);
      ...
      void scope
        .add(
          webviewWindow.listen<{
            ...
            if (event.payload.status === "synced" && changedParent === currentPath && !operation)
      ...
      void scope
        .add(
          webviewWindow.listen<RemoteFileTransferSession>("remote-file-transfer", (event) => {
            ...
            if (event.payload.status === "completed") {
              notify(
                event.payload.direction === "upload"
                  ? t("已上传 {name}", language, { name: event.payload.name })
                  : t("已下载 {name}", language, { name: event.payload.name }),
                "success",
              );
              if (
                event.payload.direction === "upload" &&
        ...
        .then(async () => {
          for (const session of await bridge.listRemoteFileTransfers())
            if (!scope.disposed) upsertTransfer(session);
        })
        .catch((error) => notify(String(error), "error"));
    }
    return () => {
  ...
  function updateDragTarget(physicalX: number, physicalY: number) {
    const element = document.elementFromPoint(
      physicalX / webviewScaleFactor,
      physicalY / webviewScaleFactor,
    );
    dragTargetPath =
      element?.closest<HTMLElement>("[data-remote-drop-path]")?.dataset.remoteDropPath ?? "";
  }

  function upsertTransfer(session: RemoteFileTransferSession) {
    if (
      transfers.some(
        (current) => current.id === session.id && current.updatedAtMs > session.updatedAtMs,
      )
    )
      return;
    ...
    if (
      ...
      return;
    transfers = [session, ...transfers.filter((transfer) => transfer.id !== session.id)].sort(
  ...
  async function loadState(requestedPeerId?: string) {
    ...
    try {
      ...
      if (requestedPeerId) {
        selectedPeerId = devices.some((device) => device.id === requestedPeerId)
          ? requestedPeerId
          : "";
        if (!selectedPeerId) {
          shares = [];
      ...
      if (!devices.some((device) => device.id === selectedPeerId))
        selectedPeerId = devices[0]?.id ?? "";
  ...
  async function loadShares() {
    ...
    await runOperation(tr("正在读取共享目录…"), async () => {
      ...
      const previewPath =
        !bridge.isTauri() && rootEntries.some((folder) => folder.relativePath === "设计资料")
          ? "设计资料"
          : "";
      if (previewPath) {
  ...
  function rememberTreeDirectory(shareId: string, path: string, nextEntries: RemoteFileEntry[]) {
    ...
    treeDirectoryVersions.set(key, (treeDirectoryVersions.get(key) ?? 0) + 1);
    next.set(key, nextEntries);
  ...
  async function ensureTreeDirectory(shareId: string, path: string, force = false) {
    ...
    const peerId = selectedPeerId;
    const ownsReply = treeOwner.capture(peerId);
    const cacheVersion = treeDirectoryVersions.get(key) ?? 0;
    try {
      const page = await bridge.listRemoteDirectory(
        peerId,
        shareId,
      ...
      if (!ownsReply(selectedPeerId) || cacheVersion !== (treeDirectoryVersions.get(key) ?? 0))
        return;
      rememberTreeDirectory(shareId, path, page.entries);
    } catch (error) {
      if (!ownsReply(selectedPeerId)) return;
      pageError = String(error);
      notify(String(error), "error");
    } finally {
      if (!ownsReply(selectedPeerId)) return;
  ...
  async function toggleTreeShare(shareId: string, expand?: boolean) {
    ...
    if (shouldExpand) next.add(shareId);
  ...
  async function toggleTreeFolder(
    row: Extract<RemoteFileTreeRow, { kind: "folder" }>,
    expand?: boolean,
  ) {
    const next = new Set(expandedFolderKeys);
    const shouldExpand = expand ?? !next.has(row.key);
    if (shouldExpand) next.add(row.key);
  ...
  function entryIcon(entry: RemoteFileEntry) {
    ...
    if (
      ["png", "jpg", "jpeg", "gif", "webp", "heic", "psd", "sketch", "fig"].includes(
        extension ?? "",
      )
    )
      return Image;
  ...
  function queueVisibleThumbnails(
    nextEntries: RemoteFileEntry[],
    peerId: string,
    shareId: string,
    generation: number,
  ) {
    if (scope.disposed || !peerId || !shareId) return;
    thumbnailQueue = nextEntries.filter(isThumbnailCandidate);
    pumpThumbnails(peerId, shareId, generation);
  }

  function pumpThumbnails(peerId: string, shareId: string, generation: number) {
    if (scope.disposed || generation !== thumbnailGeneration) return;
    while (thumbnailRunning < 4 && thumbnailQueue.length) {
      const entry = thumbnailQueue.shift()!;
      const key = `${peerId}\0${shareId}\0${entry.relativePath}\0${entry.modifiedAtMs}`;
      if (thumbnailPaths.has(key) || thumbnailPending.has(key) || thumbnailUnavailable.has(key))
        continue;
      thumbnailPending.add(key);
      thumbnailRunning++;
      void bridge
        .getRemoteFileThumbnail(
          peerId,
          shareId,
          entry.relativePath,
          entry.modifiedAtMs,
        ...
        .then((path) => {
          if (scope.disposed || generation !== thumbnailGeneration) return;
          if (path) {
            const next = new Map(thumbnailPaths);
            next.set(key, convertFileSrc(path));
            while (next.size > 256) next.delete(next.keys().next().value!);
            thumbnailPaths = next;
          } else {
            thumbnailUnavailable.add(key);
            while (thumbnailUnavailable.size > 256)
              thumbnailUnavailable.delete(thumbnailUnavailable.values().next().value!);
          }
        })
        .catch(() => {
          /* A subsequent viewport change or refresh can retry. */
        })
        .finally(() => {
          thumbnailPending.delete(key);
          thumbnailRunning--;
          pumpThumbnails(selectedPeerId, selectedShareId, thumbnailGeneration);
        });
    }
  }
  ...
  async function pickUpload(folder: boolean, targetPath = currentPath) {
    ...
    if (!selectedShare?.writable) {
      notify(
        uiTranslate("该共享目录为只读。请先在远端电脑开启写入权限后再上传。", $uiLanguage),
        "error",
      );
      return;
    }
    try {
      const session = await bridge.startRemoteUpload(
        selectedPeerId,
        selectedShareId,
        targetPath,
        folder,
  ...
  async function uploadPaths(paths: string[], targetPath = currentPath) {
    ...
    if (!selectedShare?.writable) {
      notify(
        uiTranslate("无法上传：该共享目录为只读，请先在远端电脑开启写入权限。", $uiLanguage),
        "error",
      );
      return;
    }
    try {
      upsertTransfer(
        await bridge.startRemoteUploadPaths(selectedPeerId, selectedShareId, targetPath, paths),
      );
    } catch (error) {
      notify(String(error), "error");
    }
  ...
  async function createFolder() {
    ...
    await runOperation(tr("正在创建文件夹…"), async () => {
      const entry = await bridge.createRemoteDirectory(
        selectedPeerId,
        selectedShareId,
        parentPath,
        name,
  ...
  async function renameSelected() {
    ...
    await runOperation(tr("正在重命名…"), async () => {
      const entry = await bridge.renameRemoteEntry(
        selectedPeerId,
        selectedShareId,
        relativePath,
        name,
  ...
  async function deleteSelected() {
    ...
    await runOperation(
      deleting.length > 1 ? `正在删除 ${deleting.length} 个项目…` : tr("正在删除…"),
      async () => {
        const results = await bridge.deleteRemoteEntries(
          selectedPeerId,
          selectedShareId,
          deleting.map((entry) => entry.relativePath),
        );
        await loadDirectory(currentPath);
        const failures = results.filter((result) => result.error);
        selectedPaths = new Set(failures.map((result) => result.path));
        if (failures.length)
          notify(failures.map((result) => `${result.path}: ${result.error}`).join("\n"), "error");
  ...
  async function downloadSelected() {
    ...
    try {
      const sessions = await bridge.startRemoteDownloadEntries(
        selectedPeerId,
        selectedShareId,
        downloading.map((entry) => entry.relativePath),
      );
  ...
  async function openEntry(entry: RemoteFileEntry) {
    ...
    try {
      const result = await bridge.openRemoteEntry(
        selectedPeerId,
        selectedShareId,
        entry.relativePath,
      );
  ...
  function handleTreeContextMenu(event: MouseEvent, row: RemoteFileTreeRow) {
    ...
    void showRemoteFileTreeMenu(event, {
      ...
      callbacks: {
        ...
        expand: () =>
          row.kind === "share" ? toggleTreeShare(row.shareId, true) : toggleTreeFolder(row, true),
        collapse: () =>
          row.kind === "share" ? toggleTreeShare(row.shareId, false) : toggleTreeFolder(row, false),
  ...
  function toggleAllVisible() {
    selectedPaths = allVisibleSelected
      ? new Set()
  ...
  function sortLabel() {
    return { name: "名称", modified: "修改日期", type: "类型", size: "大小" }[sortKey];
  }
  ...
  function formatSize(entry: RemoteFileEntry) {
    return entry.kind === "folder" ? "—" : formatBytes(entry.size);
  }
  ...
  function handleKeydown(event: KeyboardEvent) {
    ...
    if ((event.metaKey || event.ctrlKey) && event.key.toLowerCase() === "a") {
      ...
      selectedShare?.writable
    ) {
      event.preventDefault();
      deleteDialogOpen = true;
    } else if (event.key === "Escape") selectedPaths = new Set();
    else if (event.key === "Enter" && selectedEntries.length === 1 && selected)
...
<main class="remote-files-page">
  <header class="remote-title-row">
    <div class="remote-title-cluster">
      ...
      <div
        class="device-selector-wrap"
        use:dismissibleDropdown={{ open: deviceMenuOpen, close: () => (deviceMenuOpen = false) }}
      >
        <button
        ...
        >
          <Monitor size={17} /><span
            >{uiTranslate(
              selectedDevice?.name ?? (loading ? "正在查找设备…" : "没有已连接桌面"),
              $uiLanguage,
            )}</span
          >
          {#if selectedDevice}<i></i><small>{uiTranslate("在线", $uiLanguage)}</small
            >{/if}<CaretDown size={13} />
        </button>
        {#if deviceMenuOpen}
          <div class="device-menu">
            {#each devices as device (device.id)}
              <button on:click={() => chooseDevice(device.id)}
                ><Monitor size={17} /><span
                  ><strong>{device.name}</strong><small
                    >{uiTranslate("已配对桌面 · 在线", $uiLanguage)}</small
                  ></span
                >{#if selectedPeerId === device.id}<Check size={15} />{/if}</button
              >
            {/each}
          </div>
        {/if}
    ...
    <label class="remote-search"
      ><MagnifyingGlass size={17} /><input
        ...
        placeholder={uiTranslate(
          selectedShare
            ? `搜索 ${currentPath.split("/").at(-1) || selectedShare.name}`
            : "搜索当前目录",
          $uiLanguage,
        )}
    ...
    >
    <button
      class="remote-icon-button"
      aria-label={uiTranslate("刷新当前文件夹", $uiLanguage)}
      title={uiTranslate("刷新当前文件夹", $uiLanguage)}
      disabled={Boolean(operation)}
      on:click={refreshCurrentDirectory}><ArrowClockwise size={18} /></button
    >
  </header>
  ...
  <div class="remote-command-row">
    <div class="navigation-buttons" aria-label={uiTranslate("目录导航", $uiLanguage)}>
      <button
        aria-label={uiTranslate("后退", $uiLanguage)}
        title={uiTranslate("后退", $uiLanguage)}
        disabled={historyIndex <= 0 || Boolean(operation)}
        on:click={() => navigateHistory(historyIndex - 1)}><ArrowLeft size={18} /></button
      >
      <button
        aria-label={uiTranslate("前进", $uiLanguage)}
        title={uiTranslate("前进", $uiLanguage)}
        disabled={historyIndex >= historyPaths.length - 1 || Boolean(operation)}
        on:click={() => navigateHistory(historyIndex + 1)}><ArrowRight size={18} /></button
      >
      <button
        aria-label={uiTranslate("上一级", $uiLanguage)}
        title={uiTranslate("上一级", $uiLanguage)}
        disabled={!currentPath || Boolean(operation)}
        on:click={() => navigateTo(parentPath())}><ArrowUp size={18} /></button
      >
    </div>
    <nav class="remote-breadcrumb" aria-label={uiTranslate("当前位置", $uiLanguage)}>
      {#if selectedShare}
        <button class:current={!currentPath} on:click={() => navigateTo("")}
          >{selectedShare.name}</button
        >
        {#each breadcrumbs as crumb, index}
          <CaretRight size={13} /><button
            class:current={index === breadcrumbs.length - 1}
            on:click={() => navigateTo(breadcrumbPath(index))}>{crumb}</button
          >
        {/each}
      {:else}<span
          >{uiTranslate(
            operation || (loading ? "正在查找设备…" : "请选择一台已连接桌面"),
            $uiLanguage,
          )}</span
        >{/if}
    </nav>
    ...
    <div class="command-actions">
      ...
      />
      <div
        class="upload-action-wrap"
        use:dismissibleDropdown={{ open: uploadMenuOpen, close: () => (uploadMenuOpen = false) }}
      >
        <button
          class="command-button primary-command"
          disabled={!selectedShare?.writable || Boolean(operation)}
          on:click={() => pickUpload(false)}
          ><UploadSimple size={17} /><span>{uiTranslate("上传", $uiLanguage)}</span></button
        >
        <button
          class="upload-caret"
          aria-expanded={uploadMenuOpen}
          aria-label={uiTranslate("上传选项", $uiLanguage)}
          disabled={!selectedShare?.writable || Boolean(operation)}
          on:click={() => (uploadMenuOpen = !uploadMenuOpen)}><CaretDown size={12} /></button
        >
        {#if uploadMenuOpen}<div class="compact-menu upload-menu">
            <button on:click={() => pickUpload(false)}
              ><FileIcon size={16} />{uiTranslate("上传文件", $uiLanguage)}</button
            ><button on:click={() => pickUpload(true)}
              ><FolderOpen size={16} />{uiTranslate("上传文件夹", $uiLanguage)}</button
            >
          </div>{/if}
      </div>
      <button
        ...
        ><FolderSimplePlus size={17} /><span>{uiTranslate("新建文件夹", $uiLanguage)}</span></button
      >
      {#if selectedEntries.length}
        <span class="command-divider"></span>
        <button
          class="command-button selection-command"
          disabled={Boolean(operation)}
          on:click={downloadSelected}
          ><DownloadSimple size={17} /><span>{uiTranslate("下载", $uiLanguage)}</span></button
        >
        <button
          class="command-button selection-command"
          disabled={selectedEntries.length !== 1 || !selectedShare?.writable || Boolean(operation)}
          on:click={openRename}
          ><PencilSimple size={17} /><span>{uiTranslate("重命名", $uiLanguage)}</span></button
        >
      {/if}
      <div
        class="more-menu-wrap"
        use:dismissibleDropdown={{ open: moreMenuOpen, close: () => (moreMenuOpen = false) }}
      >
        <button
          class="command-icon-button"
          aria-expanded={moreMenuOpen}
          aria-label={uiTranslate("更多操作", $uiLanguage)}
          on:click={() => (moreMenuOpen = !moreMenuOpen)}
          ><DotsThree size={20} weight="bold" /></button
        >
        {#if moreMenuOpen}
          <div class="compact-menu more-menu">
            <button disabled={!selectedEntries.length} on:click={downloadSelected}
              ><DownloadSimple size={16} />{uiTranslate("下载所选项目", $uiLanguage)}</button
            >
            <button
              disabled={selectedEntries.length !== 1 || !selectedShare?.writable}
              on:click={openRename}
              ><PencilSimple size={16} />{uiTranslate("重命名", $uiLanguage)}</button
            >
            <button
              class="danger"
              disabled={!selectedEntries.length || !selectedShare?.writable}
              on:click={() => {
                moreMenuOpen = false;
                deleteDialogOpen = true;
              }}><Trash size={16} />{uiTranslate("删除", $uiLanguage)}</button
            >
            <span></span><button
              on:click={() => {
                moreMenuOpen = false;
                void loadDirectory(currentPath);
              }}><ArrowClockwise size={16} />{uiTranslate("刷新", $uiLanguage)}</button
            >
          </div>
        {/if}
      </div>
    ...
    <div class="display-actions">
      <div
        class="sort-wrap"
        use:dismissibleDropdown={{ open: sortMenuOpen, close: () => (sortMenuOpen = false) }}
      >
        <button
          class:open={sortMenuOpen}
          class="sort-button"
          aria-label={uiTranslate("排序", $uiLanguage)}
          aria-expanded={sortMenuOpen}
          on:click={() => (sortMenuOpen = !sortMenuOpen)}
        >
          {#if sortDirection === "ascending"}<SortAscending size={17} />{:else}<SortDescending
              size={17}
            />{/if}<span
            >{uiTranslate("排序：", $uiLanguage)}{uiTranslate(sortLabel(), $uiLanguage)}</span
          ><CaretDown size={12} />
        </button>
        {#if sortMenuOpen}
          <div class="compact-menu sort-menu">
            {#each [["name", "名称"], ["modified", "修改日期"], ["type", "类型"], ["size", "大小"]] as item}
              <button
                class:active={sortKey === item[0]}
                on:click={() => changeSort(item[0] as SortKey)}
                ><span>{item[1]}</span>{#if sortKey === item[0]}<Check size={15} />{/if}</button
              >
            {/each}
            <span></span><button
              on:click={() => {
                ...
                sortMenuOpen = false;
                void loadDirectory(currentPath);
              }}><SortAscending size={16} />{uiTranslate("升序", $uiLanguage)}</button
            ><button
              on:click={() => {
                sortDirection = "descending";
                sortMenuOpen = false;
                void loadDirectory(currentPath);
              }}><SortDescending size={16} />{uiTranslate("降序", $uiLanguage)}</button
            >
          </div>
        {/if}
      </div>
      ...
      <div class="view-switcher" aria-label={uiTranslate("文件视图", $uiLanguage)}>
        <button
          ...
          aria-label={uiTranslate("详细信息", $uiLanguage)}
          title={uiTranslate("详细信息", $uiLanguage)}
          on:click={() => (viewMode = "details")}><ListBullets size={18} /></button
        >
        <button
          class:active={viewMode === "compact"}
          aria-label={uiTranslate("紧凑列表", $uiLanguage)}
          title={uiTranslate("紧凑列表", $uiLanguage)}
          on:click={() => (viewMode = "compact")}><Rows size={18} /></button
        >
        <button
          class:active={viewMode === "grid"}
          aria-label={uiTranslate("大图标", $uiLanguage)}
          title={uiTranslate("大图标", $uiLanguage)}
          on:click={() => (viewMode = "grid")}><GridFour size={18} /></button
        >
      </div>
    </div>
  </div>
  ...
  <div class="remote-files-body">
    <aside class="remote-source-panel">
      <div class="source-device-heading">
        <Monitor size={17} /><span
          >{uiTranslate(selectedDevice?.name ?? "远程设备", $uiLanguage)}</span
        ><i class:online={Boolean(selectedDevice)}></i>
      </div>
      ...
      <div class="source-panel-footer">
        <span>{uiTranslate("远端共享", $uiLanguage)}</span><strong
          >{shares.length} {uiTranslate("个位置", $uiLanguage)}</strong
        >
      </div>
    </aside>

    <section
      class:dragging={draggingIn}
      class="remote-file-browser"
      aria-label={uiTranslate("远程文件浏览器", $uiLanguage)}
    >
      {#if viewMode === "details"}
        <div
          class="remote-table"
          role="table"
          aria-label={uiTranslate("远程文件列表", $uiLanguage)}
        >
          <div class="remote-table-head" role="row">
            <button
              class="check-column"
              aria-label={uiTranslate("选择全部", $uiLanguage)}
              on:click={toggleAllVisible}
              ><span class:checked={allVisibleSelected}
                >{#if allVisibleSelected}<Check size={11} weight="bold" />{/if}</span
              ></button
            >
            <button class:active={sortKey === "name"} on:click={() => changeSort("name")}
              ><span>{uiTranslate("名称", $uiLanguage)}</span
              >{#if sortKey === "name"}{#if sortDirection === "ascending"}<SortAscending
                    size={14}
                  />{:else}<SortDescending size={14} />{/if}{/if}</button
            >
            <button class:active={sortKey === "modified"} on:click={() => changeSort("modified")}
              ><span>{uiTranslate("修改日期", $uiLanguage)}</span
              >{#if sortKey === "modified"}{#if sortDirection === "ascending"}<SortAscending
                    size={14}
                  />{:else}<SortDescending size={14} />{/if}{/if}</button
            >
            <button class:active={sortKey === "type"} on:click={() => changeSort("type")}
              ><span>{uiTranslate("类型", $uiLanguage)}</span
              >{#if sortKey === "type"}{#if sortDirection === "ascending"}<SortAscending
                    size={14}
                  />{:else}<SortDescending size={14} />{/if}{/if}</button
            >
            <button class:active={sortKey === "size"} on:click={() => changeSort("size")}
              ><span>{uiTranslate("大小", $uiLanguage)}</span
              >{#if sortKey === "size"}{#if sortDirection === "ascending"}<SortAscending
                    size={14}
                  />{:else}<SortDescending size={14} />{/if}{/if}</button
            >
          </div>
          <div
            class="remote-table-body"
            bind:clientHeight={viewportHeight}
            bind:clientWidth={viewportWidth}
            bind:this={scrollNode}
            role="rowgroup"
            on:scroll={handleDirectoryScroll}
            on:contextmenu={handleBackgroundContextMenu}
          >
            {#if operation}<div class="remote-empty">
                <span class="remote-spinner"></span><strong>{operation}</strong>
              </div>
            {:else if !selectedPeerId}<div class="remote-empty">
                <Monitor size={29} /><strong
                  >{uiTranslate("没有可访问的桌面设备", $uiLanguage)}</strong
                ><span
                  >{uiTranslate("请先在“设置 → 设备与连接”中连接另一台桌面。", $uiLanguage)}</span
                ><button on:click={() => loadState()}>{uiTranslate("重新查找", $uiLanguage)}</button
                >
              </div>
            {:else if !selectedShareId}<div class="remote-empty">
                <FolderOpen size={30} /><strong
                  >{uiTranslate("对方没有可用共享目录", $uiLanguage)}</strong
                ><span
                  >{uiTranslate("请在对方电脑的“设置 → 文件共享”中添加目录。", $uiLanguage)}</span
                >
              </div>
            {:else if sortedEntries.length === 0}<div class="remote-empty">
                <MagnifyingGlass size={28} /><strong
                  >{uiTranslate(search ? "没有找到文件" : "这个文件夹是空的", $uiLanguage)}</strong
                ><span
                  >{uiTranslate(
                    search ? "换一个关键词试试。" : "可通过工具栏上传或新建文件夹。",
                    $uiLanguage,
                  )}</span
                >
              </div>
            {:else}
              {#if visibleWindow.top > 0}<div
                  class="file-window-spacer"
                  style:height={`${visibleWindow.top}px`}
                  aria-hidden="true"
                ></div>{/if}
              {#each visibleEntries as entry (entry.relativePath)}
                {@const Icon = entryIcon(entry)}
                <div
                  data-remote-drop-path={entry.kind === "folder" ? entry.relativePath : undefined}
                  class:selected={selectedPaths.has(entry.relativePath)}
                  class:drop-target={draggingIn && dragTargetPath === entry.relativePath}
                  class:preparing-drag={draggingOutPath === entry.relativePath}
                  class="remote-file-row"
                  role="row"
                  tabindex="0"
                  draggable="true"
                  on:keydown={(event) => handleEntryKeydown(event, entry)}
                  on:dragstart={(event) => beginRemoteDrag(event, entry)}
                  on:click={(event) => selectEntry(event, entry)}
                  on:dblclick={() => openEntry(entry)}
                  on:contextmenu={(event) => handleEntryContextMenu(event, entry)}
                >
                  <button
                    class="check-column"
                    aria-label={uiTranslate(`选择 ${entry.name}`, $uiLanguage)}
                    on:click|stopPropagation={() => toggleEntry(entry)}
                    ><span class:checked={selectedPaths.has(entry.relativePath)}
                      >{#if selectedPaths.has(entry.relativePath)}<Check
                          size={11}
                          weight="bold"
                        />{/if}</span
                    ></button
                  ...
                  <span class="file-name-cell"
                    ><span class:has-thumbnail={Boolean(thumbnailUrl(entry))} class="entry-icon"
                      >{#if thumbnailUrl(entry)}<img src={thumbnailUrl(entry)} alt="" />{:else}<Icon
                          size={20}
                          weight={entry.kind === "folder" ? "fill" : "duotone"}
                        />{/if}</span
                    ><strong>{entry.name}</strong>{#if draggingOutPath === entry.relativePath}<small
                        >{uiTranslate("准备下载…", $uiLanguage)}</small
                      >{/if}</span
                  >
                  <span>{formatModified(entry.modifiedAtMs)}</span><span
                    >{uiTranslate(typeLabel(entry), $uiLanguage)}</span
                  ><span>{formatSize(entry)}</span>
                </div>
              {/each}
              {#if visibleWindow.bottom > 0}<div
                  class="file-window-spacer"
                  style:height={`${visibleWindow.bottom}px`}
                  aria-hidden="true"
                ></div>{/if}
              {#if loadingMore}<div class="remote-load-more" role="status">
                  <span class="remote-spinner"></span>{uiTranslate(
                    "正在加载更多文件…",
                    $uiLanguage,
                  )}
                </div>{/if}
      ...
      {:else}
        <div
          ...
          class="remote-grid-body"
          bind:clientHeight={viewportHeight}
          bind:clientWidth={viewportWidth}
          bind:this={scrollNode}
          role="listbox"
          aria-label={uiTranslate("远程文件", $uiLanguage)}
          tabindex="0"
          on:scroll={handleDirectoryScroll}
          on:contextmenu={handleBackgroundContextMenu}
        >
          ...
          {:else}
            ...
            {#each visibleEntries as entry (entry.relativePath)}
              ...
              <div
                ...
                class:selected={selectedPaths.has(entry.relativePath)}
                class:drop-target={draggingIn && dragTargetPath === entry.relativePath}
                class="remote-grid-item"
                role="option"
                aria-selected={selectedPaths.has(entry.relativePath)}
                tabindex="0"
                draggable="true"
                on:keydown={(event) => handleEntryKeydown(event, entry)}
                on:dragstart={(event) => beginRemoteDrag(event, entry)}
              ...
              >
                <button
                  ...
                  ></button
                >
                <span class:has-thumbnail={Boolean(thumbnailUrl(entry))} class="grid-entry-icon"
                  >{#if thumbnailUrl(entry)}<img src={thumbnailUrl(entry)} alt="" />{:else}<Icon
                      size={viewMode === "grid" ? 48 : 23}
                      weight={entry.kind === "folder" ? "fill" : "duotone"}
                    />{/if}</span
                >
                <span class="grid-entry-copy"
                  ><strong>{entry.name}</strong><small
                    >{formatModified(entry.modifiedAtMs)}{entry.kind === "file"
                      ? ` · ${formatSize(entry)}`
                      : ""}</small
                  ></span
                >
            ...
            {#if visibleWindow.bottom > 0}<div
                ...
                aria-hidden="true"
              ></div>{/if}
            {#if loadingMore}<div class="remote-load-more grid-load-more" role="status">
                <span class="remote-spinner"></span>{uiTranslate("正在加载更多文件…", $uiLanguage)}
              </div>{/if}
          {/if}
        </div>
      {/if}
      {#if draggingIn}<div class:readonly={!selectedShare?.writable} class="drag-feedback">
          <UploadSimple size={16} />{uiTranslate(
            selectedShare?.writable
              ? `上传到“${(dragTargetPath || currentPath).split("/").at(-1) || selectedShare?.name}”`
              : "该共享目录为只读，无法上传",
            $uiLanguage,
          )}
        </div>{/if}
  ...
  {#if transfers.length}
    <aside
      class:collapsed={!transferPanelOpen}
      class="remote-transfer-panel"
      aria-label={uiTranslate("文件传输", $uiLanguage)}
    >
      <button
        class="transfer-panel-heading"
        on:click={() => (transferPanelOpen = !transferPanelOpen)}
      >
        <span
          >{uiTranslate(
            activeTransfers.length ? `正在传输 ${activeTransfers.length} 项` : "最近传输",
            $uiLanguage,
          )}</span
        >
        <small>{uiTranslate(transferPanelOpen ? "收起" : "展开", $uiLanguage)}</small><CaretDown
          size={13}
        />
      </button>
        ...
        <div class="remote-transfer-list">
          {#each visibleTransfers as transfer (transfer.id)}
            <div
              ...
              class="remote-transfer-item"
            >
              <span class="transfer-direction"
                >{#if transfer.direction === "upload"}<UploadSimple
                    size={16}
                  />{:else}<DownloadSimple size={16} />{/if}</span
              >
              <span class="transfer-copy"
                ><strong>{transfer.name}</strong><small
                  >{uiTranslate(
                    transfer.status === "failed"
                      ? transfer.error
                      : transfer.status === "completed"
                        ? "已完成"
                        : `${formatBytes(transfer.bytesTransferred)} / ${transfer.totalBytes ? formatBytes(transfer.totalBytes) : "正在准备…"}`,
                    $uiLanguage,
                  )}</small
                ><i
                  ><b
                    class:indeterminate={!transfer.totalBytes && transfer.status === "transferring"}
                    style={`width:${transferProgress(transfer)}%`}
                  ></b></i
                ></span
              >
              {#if transfer.status === "transferring"}<button
                  on:click={() =>
                    bridge
                      .cancelRemoteFileTransfer(transfer.id)
                      .catch((error) => notify(String(error), "error"))}
  ...
  <footer class="remote-status-bar">
    <span
      >{uiTranslate(
        nextCursor ? `已加载 ${sortedEntries.length} 个项目` : `${sortedEntries.length} 个项目`,
        $uiLanguage,
      )}</span
    >{#if selectedEntries.length}<i></i><span
        >{uiTranslate("已选择", $uiLanguage)}
        {selectedEntries.length}
        {uiTranslate("个", $uiLanguage)}{selectionSize
          ? ` · ${formatBytes(selectionSize)}`
          : ""}</span
      >{/if}<span class="status-spacer"></span>{#if activeTransfers.length}<span
        >{activeTransfers.length} {uiTranslate("个传输进行中", $uiLanguage)}</span
      ><i></i>{/if}<span
      >{uiTranslate(
        viewMode === "details" ? "详细信息" : viewMode === "compact" ? "紧凑列表" : "大图标",
        $uiLanguage,
      )}</span
    >
  </footer>
...
{#if folderDialogOpen || (renameDialogOpen && selected)}
  ...
  >
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={uiTranslate(folderDialogOpen ? "新建文件夹" : "重命名", $uiLanguage)}
      tabindex="-1"
      on:click|stopPropagation
      on:keydown|stopPropagation
    >
      <h2>{uiTranslate(folderDialogOpen ? "新建文件夹" : "重命名", $uiLanguage)}</h2>
      <p>
        {uiTranslate(
          folderDialogOpen
            ? tr("在当前目录中创建一个新文件夹。")
            : t("为“{name}”输入新名称。", language, { name: selected?.name ?? "" }),
          $uiLanguage,
        )}
      </p>
      <input
        bind:value={dialogValue}
        aria-label={uiTranslate(folderDialogOpen ? "文件夹名称" : "新名称", $uiLanguage)}
        on:keydown={(event) =>
          event.key === "Enter" && (folderDialogOpen ? createFolder() : renameSelected())}
      />
      <footer>
        <button
          on:click={() => {
        ...
        ><button class="confirm" on:click={folderDialogOpen ? createFolder : renameSelected}
          >{uiTranslate(folderDialogOpen ? "创建" : "保存", $uiLanguage)}</button
        >
      </footer>
    </div>
  </div>
...
{#if deleteDialogOpen}
  ...
  >
    <div
      ...
      role="alertdialog"
      aria-modal="true"
      aria-label={uiTranslate("删除项目", $uiLanguage)}
      tabindex="-1"
      on:click|stopPropagation
      on:keydown|stopPropagation
    >
      <h2>
        {uiTranslate(
          selectedEntries.length > 1
            ? `删除 ${selectedEntries.length} 个项目？`
            : t("删除“{name}”？", language, { name: selected?.name }),
          $uiLanguage,
        )}
      </h2>
      <p>{uiTranslate("所选项目将从远程电脑永久删除，此操作无法撤销。", $uiLanguage)}</p>
      <footer>
        <button on:click={() => (deleteDialogOpen = false)}
          >{uiTranslate("取消", $uiLanguage)}</button
        ><button class="delete-confirm" on:click={deleteSelected}
          >{uiTranslate("删除", $uiLanguage)}</button
        >
      </footer>
    </div>
  </div>
...
<style>
  .remote-files-page {
    position: relative;
    display: grid;
    grid-template-rows: 76px 54px minmax(0, 1fr) 30px;
    grid-template-areas: "title" "commands" "files" "status";
    width: 100%;
    height: 100%;
    min-width: 0;
    min-height: 0;
  ...
  .remote-title-row {
    grid-area: title;
    display: grid;
    grid-template-columns: minmax(0, 1fr) minmax(230px, 340px) 38px;
    align-items: center;
    gap: 12px;
    padding: 0 22px 0 28px;
    border-bottom: 1px solid var(--border);
  }
  .remote-title-cluster {
    display: flex;
    min-width: 0;
    align-items: center;
    gap: 22px;
  ...
  .remote-breadcrumb button:hover {
    color: var(--accent-strong);
    background: var(--surface-hover);
  }
  .remote-breadcrumb button.current {
    color: var(--text);
    font-weight: 620;
  }
  .command-actions,
  ...
  .command-button :global(svg),
  .sort-button :global(svg),
  .upload-caret :global(svg),
  .command-icon-button :global(svg) {
    display: block;
    flex: 0 0 auto;
  }
  ...
  .upload-caret:hover:not(:disabled) {
    background: var(--surface-hover);
  }
  .upload-menu {
    left: 0;
  ...
  .compact-menu button:hover:not(:disabled),
  .compact-menu button.active {
    background: var(--surface-hover);
  }
  .compact-menu button.danger {
    color: var(--danger);
  }
  .compact-menu > span {
    display: block;
  ...
  .source-device-heading span {
    overflow: hidden;
    text-overflow: ellipsis;
    white-space: nowrap;
  }
  .source-device-heading i {
    width: 7px;
    height: 7px;
    margin-left: auto;
    border-radius: 50%;
  ...
  .source-device-heading i.online {
    background: var(--success);
  }
  .remote-source-panel .share-tree {
    min-height: 0;
    padding: 9px 8px;
    overflow: auto;
  }
  .share-tree-row {
    ...
    align-items: center;
    padding-left: calc(3px + var(--tree-depth) * 14px);
    border-radius: 7px;
    box-sizing: border-box;
    color: var(--text-secondary);
  }
  .share-tree-row:hover {
    background: var(--surface-hover);
  }
  ...
  .tree-disclosure:hover:not(:disabled) {
    background: color-mix(in srgb, var(--surface-hover) 65%, var(--border));
  }
  .tree-disclosure :global(svg) {
    display: block;
    transition: transform 120ms ease;
  }
  .tree-disclosure.expanded :global(svg) {
    transform: rotate(90deg);
  }
  .tree-disclosure.hidden {
    visibility: hidden;
    pointer-events: none;
  }
  .tree-label {
  ...
  .remote-file-row {
    ...
    grid-template-columns:
      42px minmax(220px, 1.75fr) minmax(145px, 0.95fr) minmax(105px, 0.75fr)
      86px;
    align-items: center;
  }
  .remote-table-head {
  ...
  .remote-table-head > button:not(.check-column) {
    display: flex;
    height: 100%;
    align-items: center;
    gap: 5px;
  ...
  .remote-table-head > button:hover,
  .remote-table-head > button.active {
    color: var(--text);
    background: var(--surface-hover);
  }
  .remote-table-body {
  ...
  .remote-file-row.selected {
    background: var(--surface-selected);
  }
  .remote-file-row.selected:hover {
    background: color-mix(in srgb, var(--surface-selected) 78%, var(--surface-hover));
  }
  .remote-file-row.preparing-drag {
    opacity: 0.68;
  }
  .remote-file-row.drop-target,
  .remote-grid-item.drop-target {
    outline: 2px solid var(--accent);
    outline-offset: -2px;
    background: var(--accent-soft);
  }
  .remote-file-row > span:not(.file-name-cell) {
    padding: 0 8px;
    overflow: hidden;
    text-overflow: ellipsis;
    white-space: nowrap;
  ...
  .check-column > span.checked,
  .grid-checkbox > span.checked {
    border-color: var(--accent);
  ...
  .remote-grid-item:hover {
    background: var(--surface-hover);
  }
  .remote-grid-item.selected {
    border-color: color-mix(in srgb, var(--accent) 28%, var(--border));
    background: var(--surface-selected);
  }
  .remote-grid-body.compact .remote-grid-item {
    height: 48px;
    box-sizing: border-box;
  ...
  .remote-grid-item:hover .grid-checkbox,
  .remote-grid-item.selected .grid-checkbox {
    display: block;
  }
  .grid-entry-icon {
    display: grid;
  ...
  .remote-grid-body.compact .grid-entry-icon {
    width: 30px;
    height: 30px;
  }
  .grid-entry-copy {
  ...
  .grid-entry-copy strong,
  .grid-entry-copy small {
    overflow: hidden;
    text-overflow: ellipsis;
    white-space: nowrap;
  }
  ...
  .remote-spinner {
    ...
    border-radius: 50%;
    animation: remote-spin 800ms linear infinite;
  }
  .remote-load-more {
    display: flex;
    min-height: 48px;
  ...
  .remote-load-more .remote-spinner {
    width: 16px;
    height: 16px;
    border-width: 2px;
  }
  ...
  .remote-transfer-panel.collapsed {
    width: 220px;
  }
  ...
  .transfer-panel-heading span {
    margin-right: auto;
  }
  .transfer-panel-heading small {
    color: var(--text-muted);
    font-size: 10px;
    font-weight: 500;
  }
  .remote-transfer-panel:not(.collapsed) .transfer-panel-heading :global(svg) {
    transform: rotate(180deg);
  }
  .remote-transfer-list {
    display: grid;
    max-height: 270px;
    overflow: auto;
  }
  .remote-transfer-item {
    display: grid;
  ...
  .remote-transfer-item.failed .transfer-direction {
    color: var(--danger);
    background: var(--danger-soft);
  }
  .remote-transfer-item.completed .transfer-direction {
    color: var(--success);
  }
  .transfer-copy {
    display: grid;
  ...
  .transfer-copy b.indeterminate {
    width: 35% !important;
    animation: remote-transfer-indeterminate 1.1s ease-in-out infinite;
  }
  .remote-transfer-item em {
    color: var(--text-muted);
    font-size: 9px;
    font-style: normal;
  }
  .remote-transfer-item.failed em {
    color: var(--danger);
  }
  .remote-transfer-item.completed em {
    color: var(--success);
  }
  .remote-status-bar {
    grid-area: status;
  ...
  .remote-dialog footer .confirm {
    border-color: var(--accent);
    color: var(--text-inverse);
    background: var(--accent);
  }
  .remote-dialog footer .delete-confirm {
  ...
  @keyframes remote-transfer-indeterminate {
    from {
  ...
  @media (max-width: 1240px) {
    ...
    .command-actions .command-button span,
    .sort-button span {
      display: none;
    }
    .remote-files-body {
  ...
  @media (max-width: 980px) {
    .remote-title-row {
      grid-template-columns: minmax(0, 1fr) minmax(190px, 260px) 38px;
      padding-left: 18px;
    }
    .remote-title-cluster {
      gap: 12px;
    ...
    .remote-table-head,
    .remote-file-row {
      grid-template-columns: 38px minmax(190px, 1.5fr) 130px 85px;
    }
    .remote-table-head > :last-child,
