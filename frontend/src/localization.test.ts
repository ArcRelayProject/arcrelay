...
test("built-in preset metadata is translated while custom names and groups remain unchanged", () => {
  const rust = readFileSync(new URL("../../src/application/preset_catalog.rs", import.meta.url), "utf8");
  const presets = [...rust.matchAll(/preset\(\s*"([^"]+)",\s*"([^"]+)",\s*"([^"]+)",\s*"[^"]+",\s*"[^"]+",\s*"([^"]+)"/g)]
    .map((match) => ({ id: match[1], name: match[2], description: match[3], group: match[4] }));
  presets.push(...[...rust.matchAll(/"(path\.[^"]+)",\s*"([^"]+)",\s*"([^"]+)"/g)]
    .map((match) => ({ id: match[1], name: match[2], description: match[3], group: "文件" })));
...
test("input sharing templates preserve peer names and use complete localized messages", () => {
  const snapshot = { serviceInstanceId: "local", nearbyPeers: [
    { serviceInstanceId: "remote", displayName: "保存" },
  ], configuration: { layout: { displays: {} } } } as unknown as RuntimeSnapshot;
  ...
  assert.equal(t("{screens} 块屏幕 · {edges} 条跨屏边缘 · {devices} 台远程设备在线", "enUs",
    { screens: 3, edges: 1, devices: 1 }), "Screens: 3 · Screen edges: 1 · Remote devices online: 1");
  assert.equal(t("等待{part0}重新连接", "enUs", { part0: "我的设备 {part0}" }), "Waiting for 我的设备 {part0} to reconnect");
});
