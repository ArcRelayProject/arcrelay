...
mod tests {
    ...
    fn activated_clipboard_panel_waits_for_original_application() {
        let mut target = ClipboardPasteTarget::new(Some(42));
        assert_eq!(target.observe(None, true, ms(20)), Ok(false));
        assert_eq!(target.observe(None, false, ms(40)), Ok(false));
        assert_eq!(target.observe(Some(42), false, ms(60)), Ok(false));
        assert_eq!(target.observe(Some(42), false, ms(100)), Ok(false));
        assert_eq!(target.observe(Some(42), false, ms(200)), Ok(false));
        assert_eq!(target.observe(Some(42), false, ms(220)), Ok(true));
    }

    #[test]
    fn missing_target_is_resolved_after_panel_orders_out() {
        let mut target = ClipboardPasteTarget::new(None);
        assert_eq!(target.observe(None, false, ms(20)), Ok(false));
        assert_eq!(target.observe(Some(42), false, ms(40)), Ok(false));
        assert_eq!(target.observe(Some(42), false, ms(180)), Ok(false));
        assert_eq!(target.observe(Some(42), false, ms(200)), Ok(true));
    ...
    fn no_external_application_never_becomes_ready() {
        ...
        for elapsed in [20, 100, 500] {
            assert_eq!(target.observe(None, false, ms(elapsed)), Ok(false));
        }
    }
    ...
    fn switching_to_another_application_cancels_restoration() {
        let mut target = ClipboardPasteTarget::new(Some(42));
        assert_eq!(
            target.observe(Some(99), false, ms(20)),
    ...
    fn losing_the_restored_target_cancels_paste() {
        for next_pid in [None, Some(99)] {
            let mut target = ClipboardPasteTarget::new(Some(42));
            assert_eq!(target.observe(Some(42), false, ms(20)), Ok(false));
            ...
            assert_eq!(target.observe(next_pid, false, ms(40)), Err(expected));
        }
    }

    #[test]
    fn pinned_panel_restore_is_explicit() {
        let target = ClipboardPasteTarget::new(Some(42)).with_pinned_panel_restore(true);
    ...
    fn pinned_panel_must_relinquish_focus_for_full_settle_interval() {
        let mut target = ClipboardPasteTarget::new(Some(42));
        assert_eq!(target.observe(Some(42), true, ms(20)), Ok(false));
        assert_eq!(target.observe(Some(42), false, ms(40)), Ok(false));
        assert_eq!(target.observe(Some(42), true, ms(60)), Ok(false));
        assert_eq!(target.observe(Some(42), false, ms(80)), Ok(false));
        assert_eq!(target.observe(Some(42), false, ms(120)), Ok(false));
        assert_eq!(target.observe(Some(42), false, ms(220)), Ok(false));
        assert_eq!(target.observe(Some(42), false, ms(240)), Ok(true));
    }
}
